...
class DataTransformer:
    ...
    def set_up(self):
        ...
        for i in range(self.config.div_train_IDMap, self.config.div_test_IDMap + 1):
            for case in self.list_failureCases_solution:
                if "_IDMap{:05d}".format(i) in case:
                    # print(case)
                    self.list_sol_training.append(case)

        for i in range(self.config.div_test_IDMap, self.config.div_valid_IDMap + 1):
            for case in self.list_failureCases_solution:
                if "_IDMap{:05d}".format(i) in case:
                    # print(case)
                    self.list_sol_test.append(case)

        for i in range(self.config.div_valid_IDMap, self.config.maxNum_Map + 1):
            for case in self.list_failureCases_solution:
                if "_IDMap{:05d}".format(i) in case:
                    # print(case)
    ...
    def load_ExpertSolution(self, mode, ID_case):
        ...
        id_sol_case = name_solution_file.split('_IDCase')[-1].split('.')[0]
