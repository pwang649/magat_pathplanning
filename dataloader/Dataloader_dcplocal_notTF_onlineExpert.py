...
class CreateDataset(data.Dataset):
    def __init__(self, config, mode):
        ...
        if self.config.id_env == None:
            ...
            self.datapath_exp = '{}{:02d}x{:02d}_density_p{}/IDMap{:05d}/{}_Agent/'.format(self.config.map_type, self.config.map_w,
                                                                               self.config.map_h,
                                                                               self.config.map_density, self.config.id_env,
    ...
    def __getitem__(self, index):
        ...
        input, target, GSO, map_tensor,ID_Map, ID_case = self.load_data(path, id_step)
        return input, target, (ID_Map, ID_case, id_step), GSO, map_tensor
    ...
    def update_data_path_trainingset(self, dir_data):
        ...
        if self.config.train_TL:
            data_paths_pretrained, step_paths_pretrained = self.search_files(self.config.failCases_dir_load)
            data_paths_total.extend(data_paths_pretrained)
            step_paths_total.extend(step_paths_pretrained)
    ...
    def load_train_data(self, path, id_step):
        ...
        return step_input_tensor, step_target, step_input_GSO, tensor_map, ID_Map,ID_case
